...
<manifest>
  <remote name="github" fetch="git@github.com:AfterShip/" review="https://github.com/AfterShip/" />
  <default remote="github" revision="master" sync-j="4" />
  <project name="feed-api" path="services/feed-api" groups="feed,be" />
  <project name="products-center" path="services/products-center" groups="products" />
</manifest>`
...
<manifest>
  ...
  <default push="fork" />
  <project name="products-center" revision="develop" />
</manifest>`
	...
	for _, p := range merged.Projects {
		if p.Name == "products-center" {
			if p.Revision != "develop" {
				t.Errorf("expected products-center revision=develop, got %q", p.Revision)
			}
			if p.Path != "services/products-center" {
